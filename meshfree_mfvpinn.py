...
class Config:
    ...
    ANCHOR_GROW_POLICY: str = "fraction"
...
# Optional deterministic L-BFGS (FIRST + POLISH ONLY)
        ...
        # (B) Strategy-1 marking rule (fig3_strategy1-style)
        if grow_policy == "strategy1":
            ...
            if marked_core.size >= parents_needed:
                ...
                if fill_global:
                    # Fill parents with next-best anchors by eta (keeps deterministic ordering)
        ...
        if grow_policy == "strategy1" and (marked_ids is not None):
...
def main(argv=None):
    ...
    parser.add_argument("--anchor-growth-policy", type=str, default="fraction",
                        choices=["fraction", "strategy1", "seq"],
                        help="How many anchors to add each outer step. 'strategy1' follows fig3_strategy1 marking logic; 'seq' forces a fixed total-anchor schedule.")
    ...
    if int(getattr(args, "run_both_cm", 0)) == 1:
        ...
        if not args.no_plot:
            try:
                ...
                out_png = os.path.join(outdir, "figure3_patchless.png")
                plot_figure3_compare(curves, out_png=out_png, title="PATCHLESS Kernelized MF-VPINN (RBF, meshfree)")
