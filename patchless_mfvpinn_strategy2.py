...
def _flatten_grads(grads, vars_):
    ...
    return flat_tf.numpy().astype(np.float32)
...
class Config:
    ...
    ANCHOR_GROW_POLICY: str = "strategy1"
...
def run_patchless(cfg: Config, outdir: str, lift: str, seed_base: int,
    ...
    for it in range(int(cfg.OUTER_ITERS)):
        ...
        if grow_policy in ("strategy1", "strategy2"):
        ...
        if marked_ids is not None:
            ...
            cand_seed = int(cfg.CAND_SEED + seed_base) if int(getattr(cfg, 'FREEZE_CANDIDATES', 0)) == 1 else int(
                cfg.CAND_SEED + seed_base + 31 * it)
...
def main(argv=None):
    ...
    parser.add_argument("--anchor-growth-policy", type=str, default="fraction",
                        choices=["fraction", "strategy1", "strategy2", "seq"],
...
# ===== Public aliases for safe external use =====

def reflect_unit_square_np(x):
    return _reflect_unit_square_np(x)

def strategy1_children_candidates_np(*args, **kwargs):
    return _strategy1_children_candidates_np(*args, **kwargs)

def strategy2_children_candidates_np(*args, **kwargs):
    return _strategy2_children_candidates_np(*args, **kwargs)

def strategy2_fixed_offsets(*args, **kwargs):
    return _strategy2_fixed_offsets(*args, **kwargs)

def zeros_like(x):
    return _zeros_like(x)

def safe_add_grads(a, b):
    return _safe_add_grads(a, b)
