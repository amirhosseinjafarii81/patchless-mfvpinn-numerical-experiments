...
Patchless / coverless MF-VPINN, Strategy #4.

This script is a faithful Strategy-4 adaptation of the patchless Strategy-1 codebase:
- keep the same meshfree RKHS / kernelized-anchor MF-VPINN formulation,
- keep the same training / H1-evaluation / KDTree / snapshot infrastructure,
- BUT change the parent-marking rule from the normalized error indicator
      eta_a = (R_a / sqrt(G_a + eps))^2
  to the raw squared anchor residual
      r_a^2 = R_a^2,
  in direct analogy with the patch-based `fig13_strategy4.py` script, where
  patches are marked by `r_{h,i}^2` instead of the error indicator.

Patchless Strategy-4 interpretation used here:
- Parents are selected by Dörfler marking on raw anchor residual squares `R_a^2`.
- Child anchors are still created with the Strategy-1 CM-style local geometry.
- To stay faithful to the "without the error indicator" ablation, local child
  placement and fallback filling use uniform/random selection (`use_eta=False`).
- Snapshots store raw per-anchor residual squares as
      P{M}_centers, P{M}_h, P{M}_r2
  so they can be plotted by the companion `patchless_fig14_15_strategy4.py`.

The rest of the solver is intentionally kept aligned with `meshfree_mfvpinn.py`
so that differences come from the Strategy-4 marking rule, not from unrelated
...
class KDTreePruner:
    ...
    def enable_stencil_bank(self, Smax: int, K: int = 4, seed: int = 1234, force_rebuild: bool = False):
        ...
        need = need or (int(self._stencil_Smax) != Smax) or (int(self._stencil_K) != K) or (int(self._stencil_seed) != seed)
...
class Config:
    ...
    ANCHOR_GROW_POLICY: str = "strategy4"  # strategy1 | strategy4 | fraction | seq
...
def kernel_eval_precompute(*args, **kwargs):
    ...
    if len(args) == 4 and hasattr(args[0], "KERNEL_KIND"):
        ...
        cutoff_s = tf.constant(cutoff_factor * cutoff_factor, dtype=DTYPE) if (cutoff_factor and cutoff_factor > 0.0) else None
...
def adam_train_cached_kdtree(unet, gnet, lift_mode: int,
                             ...
                             seed: int,
                             pruner: KDTreePruner,
                             cutoff_factor: float = 4.0, lr0_override: float = None, lr1_override: float = None, deterministic: int = None):
    """
        ...
        for _ in range(ga):
            ...
            while True:
                try:
                    ...
                    aids = (((_it * ga + _) * A + np.arange(A, dtype=np.int64)) % M).astype(np.int32) if det == 1 else rng.integers(0, M, size=A, dtype=np.int32)
                    ...
                    pids_e = (((_it * ga + _) * pb_e + np.arange(pb_e, dtype=np.int64)) % Nq).astype(np.int32) if det == 1 else rng.integers(0, Nq, size=pb_e, dtype=np.int32)
        ...
        while True:
            try:
                ...
                for _acc in range(ga):
                    ...
                    pids = ((step_idx * pb + np.arange(pb, dtype=np.int64)) % Nq).astype(np.int32) if det == 1 else rng.integers(0, Nq, size=pb, dtype=np.int32)
                    aids = ((step_idx * ab + np.arange(ab, dtype=np.int64)) % M).astype(np.int32) if det == 1 else rng.integers(0, M, size=ab, dtype=np.int32)
...
def giqmc_weights_from_eta(uv: np.ndarray, jac: np.ndarray, eta: np.ndarray, cfg: Config,
                          ema_state: Dict[str, np.ndarray] = None, key: str = "h1") -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
...
def train_gnet_boundary(gnet, cfg: Config, seed=0):
    ...
    for k in range(1, steps + 1):
        ...
        if cfg.GNET_PRINT_EVERY and (k % int(cfg.GNET_PRINT_EVERY) == 0):
            print(f"    [gnet] step {k:05d}  l2={float(l2.numpy()):.3e}  top={float(ltop.numpy()):.3e}  loss={float(loss.numpy()):.3e}")
...
def _basis_novelty_projection(anchors_np, ell_np, shortlist_idx, cfg):
    ...
    with tf.device("/CPU:0"):
        for t, i in enumerate(shortlist_idx.tolist()):
            ...
            ell_local_base = _np.maximum(ell_np[local_ids, 0].astype(_np.float64, copy=True), ell_min if ell_min > 0 else 0.0)
...
def add_anchors_strategy1_cm_meshfree(unet, gnet, lift_mode: int,
                                     anchors: np.ndarray, ell: np.ndarray, aw: np.ndarray,
                                     marked_ids: np.ndarray,
                                     cm: int, add_count: int,
                                     cfg: Config,
                                     bubble_mode_tf, softmin_tau_tf, bubble_power_tf,
    ...
    if add_count <= 0 or marked_ids.size == 0:
        return anchors, ell, aw, {"added": 0, "tries": 0, "rejects": 0, "base_ell": float("nan"), "min_sep": float("nan")}
    ...
    # Propose local candidates around effective parents
    Z, ell_Z, _parent_anchor_id = _strategy1_children_candidates_np(
        rng, anchors, ell, parents_eff, cm=cm, cfg=cfg, base_ell=base_ell
    )
    N = int(Z.shape[0])
    if N <= 0:
        return anchors, ell, aw, {"added": 0, "tries": 0, "rejects": 0, "base_ell": base_ell, "min_sep": float(cfg.MIN_SEP_ALPHA) * base_ell}
...
def add_anchors_adaptive(unet, gnet, lift_mode: int,
    ...
    if cfg.USE_MULTI_SCALE:
        ...
        ell_small = float(max(cfg.ELL_MIN, cfg.ELL0_SMALL * (float(M0) / float(max(1, M + new_M))) ** float(cfg.ELL_SCHED_P)))
...
def lbfgs_optimize(
    ...
    def fun_and_jac(x):
        ...
        while True:
            try:
                ...
                print(f"[L-BFGS][OOM-backoff][stream] reducing batches: point_batch={pbp_cur}, edge_batch={eb_cur}", flush=True)
                continue
    ...
    options = dict(maxiter=int(maxiter), maxcor=int(cfg.LBFGS_MAXCOR), ftol=float(cfg.LBFGS_FTOL), gtol=gtol, maxfun=maxfun)
    res = sopt.minimize(
        fun_and_jac,
        x0,
        method="L-BFGS-B",
    ...
    if hasattr(res, "message"):
        ...
        msg = "done"
    print(f"[L-BFGS] STOP: {msg}  nit={getattr(res,'nit',None)} nfev={getattr(res,'nfev',None)} fun={getattr(res,'fun',None):.3e}")
...
def run_patchless(cfg: Config, outdir: str, lift: str, seed_base: int,
    ...
    print("\n>>> Starting PATCHLESS Kernelized MF-VPINN Strategy #4 (no mesh, no patch cover)")
    ...
    for it in range(int(cfg.OUTER_ITERS)):
        ...
        for _s in range(ramp_stages):
            ...
            _seed_local = seed_base if getattr(cfg, 'DETERMINISTIC_BATCHING', False) else (seed_base + 1000 + it * 17 + _s)
        ...
        if int(getattr(cfg, "H1_MONO_ENFORCE", 0)) == 1 and (prev_h1 is not None) and (it > 0):
            ...
            while (h1_cur > target) and (phase < max_phases):
                ...
                _seed_local = seed_base if getattr(cfg, 'DETERMINISTIC_BATCHING', False) else (seed_base + 90000 + it * 97 + phase)
                _run_adam(extra_steps, lr_scale=lr_scale, seed_local=_seed_local)
                h1_new = float(h1_eval.rel_H1(unet, gnet, lift_mode_int,
                                              bubble_mode_tf, softmin_tau_tf, bubble_power_tf,
                                              ref_energy_const=h1_ref_energy))
                if verbose:
                    print(f"    [H1-guard] it={it:02d} phase={phase+1}/{max_phases} lr_scale={lr_scale:.3g} H1={h1_cur:.3e} -> {h1_new:.3e} (target<={target:.3e})", flush=True)
                h1_cur = h1_new
                phase += 1
            ...
            if h1_cur > target:
                ...
                if int(getattr(cfg, "H1_MONO_HARD", 1)) == 1:
                    ...
                    while (h1_cur > target) and (spent < hard_budget):
                        ...
                        _seed_local = seed_base if getattr(cfg, 'DETERMINISTIC_BATCHING', False) else (seed_base + 95000 + it * 137 + hard_round)
                        ...
                        h1_new = float(h1_eval.rel_H1(unet, gnet, lift_mode_int,
                                                      ...
                                                      ref_energy_const=h1_ref_energy))
                        if hard_verbose:
                            print(f"    [H1-hard] it={it:02d} round={hard_round+1} spent={spent}/{hard_budget} lr_scale={lr_scale:.3g} H1={h1_cur:.3e} -> {h1_new:.3e} (target<={target:.3e})", flush=True)
                        h1_cur = h1_new
                        hard_round += 1

                    if h1_cur > target:
                        print(f"[STOP] H1 monotone hard-enforce failed: H1={h1_cur:.3e} target<={target:.3e} after spent={spent} steps. Stopping run (no rollback).", flush=True)
                        break
                else:
                    print(f"[WARN] H1 monotone guard failed: H1={h1_cur:.3e} target<={target:.3e}. Continuing refinement (no rollback).", flush=True)
...
# Optional deterministic L-BFGS (FIRST + POLISH ONLY)
        ...
        h1_raw0 = float(h1_after_train) if (h1_after_train is not None) else float(h1_eval.rel_H1(unet, gnet, lift_mode_int,
                                 bubble_mode_tf, softmin_tau_tf, bubble_power_tf,
                                 ref_energy_const=h1_ref_energy))
        ...
        time_hist.append(dt)
        print(f"Step {it:02d}: M={M:4d} | loss={ev['loss']:.3e} | H1_raw={h1_raw0:.3e} | H1_end={h1_rep:.3e} | H1_best={best_h1:.3e} | ell≈{float(np.median(ell)):.3g} | time[s]={dt:.1f}")
        ...
        # ------------------------------------------------------------
        # Snapshot capture (Strategy #4 schema)
        # Keys: P{M}_centers, P{M}_h, P{M}_r2
        # ------------------------------------------------------------
        if snapshot_every and (it % int(snapshot_every) == 0):
            # Strategy #4 uses the *raw* squared anchor residuals for marking.
            Rabs = np.asarray(ev.get("Rabs", np.zeros((M,), dtype=np.float64)), dtype=np.float64).reshape(-1)
            r2_a = np.asarray(Rabs * Rabs, dtype=np.float64).reshape(-1)

            snaps[f"P{M}"] = {
                "centers": np.asarray(anchors, dtype=np.float64).copy(),  # (M,2)
                "h": np.asarray(ell, dtype=np.float64).copy().reshape(-1),  # (M,)
                "r2": np.asarray(r2_a, dtype=np.float64).copy().reshape(-1),  # (M,)
            }
        ...
        grow_policy = str(getattr(cfg, "ANCHOR_GROW_POLICY", "fraction"))
        mark_stats = None
        marked_ids = None
        add_count = 0
        ...
        if grow_policy == "seq" and seq:
            ...
            if (it + 1) < len(seq):
                ...
                # schedule ended: gracefully fall back to Strategy #4 marking
                grow_policy = "strategy4"

        # (B) Strategy-1 / Strategy-4 marking rules
        if grow_policy in ("strategy1", "strategy4"):
            if grow_policy == "strategy4":
                # Strategy #4: mark parents by the raw squared anchor residuals R_a^2,
                # not by the normalized error indicator eta_a = R_a^2 / G_a.
                Rabs = np.asarray(ev.get("Rabs", np.zeros((M,), dtype=np.float64))).reshape(-1)
                weights = np.asarray(Rabs * Rabs, dtype=np.float64).reshape(-1)
            else:
                # Strategy #1 baseline: normalized residual-energy indicator.
                Rabs = np.asarray(ev.get("Rabs", np.zeros((M,), dtype=np.float64))).reshape(-1)
                ...
                if (gsm != "eta") and (beta > 0.0):
                    ...
                    if gsm == "projection":
                        ...
                        nwant = int(min(room, max(int(getattr(cfg, 'MIN_ADD_PER_ITER', 1)), math.ceil(float(getattr(cfg, 'ADD_FRAC_MAX', 0.0)) * M))))
                        nwant = int(min(nwant, int(getattr(cfg, 'MAX_ADD_PER_ITER', nwant))))
            ...
            # by taking additional high-score anchors globally (if enabled).
            # ------------------------------------------------------------------
            ...
            if marked_core.size >= parents_needed:
                ...
                if fill_global:
                    # Fill parents with the next-best anchors according to the current score.
                    marked_ids = idx[:parents_needed].astype(np.int32, copy=False)
                else:
        ...
        if grow_policy == "fraction":
            add_max = int(min(cfg.MAX_ADD_PER_ITER, max(cfg.MIN_ADD_PER_ITER, math.ceil(cfg.ADD_FRAC_MAX * M))))
        ...
        if grow_policy in ("strategy1", "strategy4") and (marked_ids is not None):
            # CM-style local refinement around marked *existing* anchors (meshfree, patchless).
            # Strategy #4 keeps the same local geometry as Strategy #1 but disables indicator-based
            # candidate ranking, so child placement is effectively random/uniform inside the marked supports.
        ...
        use_eta=((grow_policy != "strategy4") and (str(getattr(cfg, "ADAPTIVE_MODE", "eta")) == "eta")),
            )
        else:
            # Refresh candidate pool each iteration (prevents growth deadlock when Poisson min-sep
            # makes most of a fixed pool unusable)
            cand_seed = int(cfg.CAND_SEED + seed_base) if int(getattr(cfg, 'FREEZE_CANDIDATES', 0)) == 1 else int(cfg.CAND_SEED + seed_base + 31*it)
        ...
        print(f"         grow[{getattr(cfg,'ANCHOR_GROW_POLICY','fraction')}]: req={add_count} added={info['added']} tries={info['tries']} reject_rate={rej_rate*100:.1f}%  min_sep={info.get('min_sep', float('nan')):.3g}  base_ell={info.get('base_ell', float('nan')):.3g}  eta_mean={info.get('eta_mean', float('nan')):.2e}  eta_p95={info.get('eta_p95', float('nan')):.2e}{extra}")
    ...
    if final_lbfgs_iters > 0:
        ...
        try:
            ...
            print(f"[FINAL POLISH] M={M_pol:4d} | loss={ev_pol['loss']:.3e} | H1_raw={h1_raw0_pol:.3e} | best={best_h1:.3e} | improved={improved} | time[s]={dt_pol:.1f}", flush=True)
        except Exception as e:
    ...
    # Strategy-4 convenience aliases (same naming style as the patch-based scripts).
    cm_tag = int(getattr(cfg, "ANCHOR_CM", -1))
    if cm_tag > 0:
        np.savez(
            os.path.join(outdir, f"history_strategy4_CM{cm_tag}.npz"),
            N=np.asarray(x_hist, np.int64),
            H1=np.asarray(h1_raw_hist, np.float64),
            H1_raw=np.asarray(h1_raw_hist, np.float64),
            H1_end=np.asarray(h1_hold_hist, np.float64),
            H1_best=np.asarray(h1_best_hist, np.float64),
            loss=np.asarray(loss_hist, np.float64),
            cm=np.asarray([cm_tag], np.int64),
        )
        save_snapshots_npz(os.path.join(outdir, f"snapshots_strategy4_CM{cm_tag}.npz"), snaps)

    return {"x": x_hist, "h1_raw": h1_raw_hist, "h1": h1_hold_hist, "h1_best": h1_best_hist, "loss": loss_hist, "time": time_hist, "snaps": snaps}


def save_snapshots_npz(path, snaps: Dict[str, Any]):
    """
    Save snapshots in a generic strategy-aware schema.

    Always writes:
      P{M}_centers : (M,2)
      P{M}_h       : (M,)

    Additionally writes any extra 1D per-anchor arrays found in each snapshot dict,
    for example:
      P{M}_r2
      P{M}_eta
      P{M}_level
    ...
    for k, v in snaps.items():
        ...
        for extra_key, extra_val in v.items():
            if extra_key in ("centers", "h"):
                continue
            arr = np.asarray(extra_val, dtype=np.float64)
            if arr.ndim == 1 or (arr.ndim == 2 and 1 in arr.shape):
                out[f"{k}_{extra_key}"] = arr.reshape(-1)
...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(description="Patchless kernelized MF-VPINN (Figure 3 benchmark)")
    ...
    parser.add_argument("--domain-normalize", type=int, default=1, help="map NN inputs from [0,1]^d to [-1,1]^d internally")
    ...
    parser.add_argument("--laplace-h", type=float, default=1e-3, help="FD step h for Laplacian indicator eta (only used when --adaptive eta).")
    ...
    parser.add_argument("--kernel-cutoff", type=float, default=4.0, help="Ignore kernel contributions when r/ell > C (C~3-5) for speed")
    ...
    parser.add_argument("--stencil-bank-k", type=int, default=4, help="Number of stencils per anchor (cycled during training).")
    ...
    parser.add_argument("--cache-energy", type=int, default=1, choices=[0,1], help="Cache G_j once per outer iteration when WHITEN_MODE=energy")
    parser.add_argument("--nested-energy", type=int, default=1, help="If 1, keep old anchor energies G_j fixed and compute G only for newly added anchors.")
    ...
    parser.add_argument("--lbfgs-point-batch", type=int, default=0, help="If >0, override point batch size inside L-BFGS (else uses --eval-point-batch).")
    parser.add_argument("--lbfgs-edge-batch", type=int, default=0, help="If >0, override edge chunk size inside L-BFGS KDTree path (else uses --eval-edge-batch).")
    parser.add_argument("--lbfgs-anchor-batch", type=int, default=0, help="If >0, override anchor batch size inside L-BFGS (else uses --eval-anchor-batch).")
    ...
    parser.add_argument("--anchor-growth-policy", type=str, default="strategy4",
                        choices=["fraction", "strategy1", "strategy4", "seq"],
                        help="How many anchors to add each outer step. 'strategy1' uses the normalized indicator eta_a, 'strategy4' marks by raw residual squares R_a^2, and 'seq' forces a fixed total-anchor schedule.")
    parser.add_argument("--mark-frac", type=float, default=0.75, help="Strategy1 marking fraction (cumulative weight target).")
    parser.add_argument("--mark-cap", type=float, default=0.30, help="Strategy1 marking cap as a fraction of current M.")
    parser.add_argument("--anchor-cm", type=int, default=4, help="Strategy1: number of children anchors per marked anchor.")
    ...
    parser.add_argument("--restore-best-after-grow", type=int, default=0, help="If 1, restore best network weights after anchor growth (stability; reporting still uses raw H1).")
    ...
    parser.add_argument("--disable-xla", type=int, default=1, choices=[0,1], help="Disable TF XLA JIT to avoid duplicate-variable XLA crashes")
    ...
    parser.add_argument("--h1-mono-hard-budget", type=int, default=4000, help="Max additional Adam steps beyond --h1-mono-max-phases loop.")
    parser.add_argument("--h1-mono-hard-extra-steps", type=int, default=200, help="Adam steps per hard-enforce round.")
    parser.add_argument("--h1-mono-hard-lr-decay", type=float, default=0.5, help="Extra LR decay factor used in hard-enforce mode.")
    parser.add_argument("--h1-mono-hard-min-lr-scale", type=float, default=0.02, help="Minimum LR scale allowed in hard-enforce mode.")
    ...
    parser.add_argument("--h1-use-giqmc", type=int, default=1, help="If 1, GI-QMC (graded + importance) H1 estimator. If 0, plain graded QMC.")
    ...
    parser.add_argument("--marking", type=str, default="eta", choices=["eta","sqrtgamma_eta","gamma_eta"], help="Marking quantity for adaptive growth (strategy1-style).")
    ...
    parser.add_argument("--growth-proj-reg", type=float, default=1e-6, help="Regularizer for local Gram solve in projection mode.")
    parser.add_argument("--growth-local-k", type=int, default=8, help="Nearest-neighbor count for local Gram (projection mode).")
    parser.add_argument("--growth-rho-eps", type=float, default=1e-12, help="Epsilon for rho normalization.")
    parser.add_argument("--growth-loc-nq", type=int, default=512, help="Local probe points per shortlisted anchor (projection mode).")
    parser.add_argument("--growth-shortlist-factor", type=float, default=4.0, help="Compute projection novelty only for top (factor * requested_add) anchors.")
    # Conditioning control (Gram stability / whitening)
    parser.add_argument("--condctrl-enable", type=int, default=cfg.CONDCTRL_ENABLE, help="Enable conditioning control for local Gram matrices (0/1)")
    parser.add_argument("--condctrl-kappa-unstable", type=float, default=cfg.CONDCTRL_KAPPA_UNSTABLE, help="Condition number threshold for 'unstable' region")
    parser.add_argument("--condctrl-kappa-reject", type=float, default=cfg.CONDCTRL_KAPPA_REJECT, help="Condition number threshold for hard reject/repair")
    parser.add_argument("--condctrl-shrink-gamma", type=float, default=cfg.CONDCTRL_SHRINK_GAMMA, help="Support shrink factor per repair step (0<gamma<1)")
    parser.add_argument("--condctrl-max-shrink", type=int, default=cfg.CONDCTRL_MAX_SHRINK, help="Max number of support-shrink repair attempts")
    parser.add_argument("--condctrl-eps0", type=float, default=cfg.CONDCTRL_EPS0, help="Minimum numerical floor used in conditioning control")
    parser.add_argument("--condctrl-alpha-lammax", type=float, default=cfg.CONDCTRL_ALPHA_LAMMAX, help="Adaptive ridge term scale relative to lam_max")
    parser.add_argument("--condctrl-alpha-tracemean", type=float, default=cfg.CONDCTRL_ALPHA_TRACEM, help="Adaptive ridge term scale relative to mean(trace)")
    parser.add_argument("--condctrl-whiten", type=int, default=cfg.CONDCTRL_WHITEN, help="Use whitening transform for stable projection (0/1)")
    parser.add_argument("--condctrl-growth-gamma", type=float, default=cfg.CONDCTRL_GROWTH_GAMMA, help="Multiply projection novelty by (lam_min/lam_max)^gamma (0 disables)")


    # misc
    parser.add_argument("--snapshot-every", type=int, default=1)
    parser.add_argument("--plot-best", type=int, default=0, help="If 1, also plot best-so-far curve as dashed line (for reference).")
    ...
    if isinstance(args.anchor_growth_seq, str) and args.anchor_growth_seq.strip():
        ...
        if seq and cfg.ANCHOR_INIT != seq[0]:
            print(f"[warn] anchor-init={cfg.ANCHOR_INIT} but anchor-growth-seq starts with {seq[0]}; using {seq[0]} as anchor-init.")
    ...
    print(f"Kernel: {cfg.KERNEL_KIND}  ell0={cfg.ELL0:g} ell_min={cfg.ELL_MIN:g} sched_p={cfg.ELL_SCHED_P:g}  cutoff={cfg.KERNEL_CUTOFF:g}")
    print(f"Anchors: init={cfg.ANCHOR_INIT} target={cfg.TARGET_NANCHOR} outer={cfg.OUTER_ITERS} add_frac_max={cfg.ADD_FRAC_MAX:g} min_add={cfg.MIN_ADD_PER_ITER}")
    print(f"QMC: {cfg.QMC_KIND} scramble={cfg.QMC_SCRAMBLE}  NQ_GLOBAL={cfg.NQ_GLOBAL}  NCAND={cfg.NCAND}")
    print(f"Adam: pb={cfg.ADAM_POINT_BATCH} ab={cfg.ADAM_ANCHOR_BATCH} grad_accum={cfg.ADAM_GRAD_ACCUM}  steps_first={cfg.ADAM_STEPS_FIRST} steps_after={cfg.ADAM_STEPS_AFTER_GROW}")
    ...
    # Run both CM values in one go, to get a single joint Strategy-4 comparison plot.
    if int(getattr(args, "run_both_cm", 0)) == 1:
        ...
        if not args.no_plot:
            try:
                ...
                out_png = os.path.join(outdir, "figure13_patchless_strategy4.png")
                plot_figure3_compare(curves, out_png=out_png, title="PATCHLESS Strategy #4 (raw residual marking)")
